...
static void
insert_domain_andexpr(ma_domain_expr& expr, ma_domain_andexpr const& andexpr)
{
  expr.insert_andexpr(andexpr);
}

static void
insert_domain_cond(ma_domain_andexpr& andexpr, ma_domain_cond const& cond)
{
  andexpr.insert_cond(cond);
}

static void
insert_domain_expr(ma_domain_cond& cond, ma_domain_expr const& expr)
{
                ...
                ma_rule* rule)
{
  ...
  rule->update_notify_list(name, (arg == 's') ? SOURCE : TARGET);
...
template <class FwdIter, class Skip>
novadaq::errorhandler::domain_expr_parser<FwdIter, Skip>::domain_expr_parser(
  ma_rule* rule)
  : domain_expr_parser::base_type(domain_expr)
{
  domain_expr =
    domain_andexpr[phx::bind(&insert_domain_andexpr, ql::_val, ql::_1)] % "OR";

  domain_andexpr =
    domain_cond[phx::bind(&insert_domain_cond, ql::_val, ql::_1)] % "AND";
  ...
  domain_cond =
    ...
    | (((key[ql::_a = ql::_1] >> ".$" >>
         char_(
           "st")[phx::bind(&insert_cond_arg, ql::_val, ql::_a, ql::_1, rule)]) %
        '=') >>
       -('=' >> (str[phx::bind(&insert_str_cond, ql::_val, ql::_1)] |
                 "ANY"))) // cond1.$x = cond2.$y = ... -( = "str_cond" )
                          // cond1.$x = cond2.$y = ... -( = ANY )
    ;

  keywords = no_case["AND"] | no_case["OR"] | no_case["ANY"];
...
insert_ext_func(ma_boolean_cond& cond,
                ...
                ma_rule* rule)
{
  cond.insert_ext_func(
    rule->insert_condition_ptr(name, false /*non-primitive*/),
...
static void
insert_compare_op_double(ma_boolean_cond& cond, compare_op_t op, double rhv)
{
  cond.insert_compare_op_double(op, rhv);
}

static void
insert_compare_op_bool(ma_boolean_cond& cond, compare_op_t op, bool rhv)
{
  if ((op != CO_E) && (op != CO_NE))
...
static void
insert_compare_op_string(ma_boolean_cond& cond, compare_op_t op, string_t rhv)
{
...
template <class FwdIter, class Skip>
novadaq::errorhandler::boolean_expr_parser<FwdIter, Skip>::boolean_expr_parser(
  ma_rule* rule)
  : boolean_expr_parser::base_type(boolean_expr)
{
  ...
  boolean_cond =
    ...
    | (lit('!') >> lit('(') >>
       ...
       ')') // '! (' >> expr >> ')'
    |
    (key[ql::_a = ql::_1] >> lit('(') >> key[ql::_d = ql::_1] >>
     -(".$" >> char_("st")[ql::_b = ql::_1]) >>
     -(',' >> args[ql::_e = ql::_1]) >>
     lit(')')[phx::bind(
       &insert_ext_func, ql::_val, ql::_a, ql::_d, ql::_b, ql::_e, rule)]

     >> -(compare_op[ql::_c = ql::_1] >>
          (double_[phx::bind(
             &insert_compare_op_double, ql::_val, ql::_c, ql::_1)] |
           bool_[phx::bind(&insert_compare_op_bool, ql::_val, ql::_c, ql::_1)] |
           str[phx::bind(&insert_compare_op_string,
                         ql::_val,
                         ql::_c,
                         ql::_1)]))) // custom_function(cond.$st)
    | key[phx::bind(&insert_primitive_cond, ql::_val, ql::_1, rule)]
    // Cond
    | (lit('!') >>
       key[phx::bind(&insert_primitive_cond_neg, ql::_val, ql::_1, rule)]);
...
bool
novadaq::errorhandler::parse_condition_expr(string_t const& s, ma_rule* rule)
{
  ...
  bool b =
    qi::phrase_parse(
      ...
      boolean_p[phx::bind(&set_boolean_expr, ql::_1, rule)] >>
        -("WHERE" >> domain_p[phx::bind(&set_domain_expr, ql::_1, rule)]),
      space) &&
    begin == end;
...
static void
insert_test_expr(ma_cond_test_primary& primary, ma_cond_test_expr const& expr)
{
...
template <class FwdIter, class Skip>
novadaq::errorhandler::cond_test_expr_parser<FwdIter,
                                             Skip>::cond_test_expr_parser()
  : cond_test_expr_parser::base_type(test_expr)
{
  ...
  test_primary =
    (lit('(') >> test_expr[phx::bind(&insert_test_expr, ql::_val, ql::_1)] >>
     lit(')')) |
    (key[ql::_a = ql::_1] >> lit('(') >> -values[ql::_b = ql::_1] >>
     lit(')')[phx::bind(&insert_test_func, ql::_val, ql::_a, ql::_b)] >>
     -(compare_op[ql::_c = ql::_1] >>
       value[phx::bind(&insert_test_compare_op, ql::_val, ql::_c, ql::_1)]));

  values = (value % ',');

  value = (int_[ql::_val = ql::_1] >> !char_(".eE")) |
          double_[ql::_val = ql::_1] | bool_[ql::_val = ql::_1] |
          str[ql::_val = ql::_1];
                                            ...
                                            ma_cond_test_expr& expr)
{
  ...
  bool b = qi::phrase_parse(begin, end, test_p, space, expr) && begin == end;
