...
bool
ma_condition::force(bool val, conds_t& status, conds_t& source, conds_t& target)
{
  if (per_source() || per_target())
    throw std::runtime_error("to force the value of a condition, it must not "
                             "be per_source and per_target");
...
bool
ma_condition::event(size_t src, size_t tgt, time_t t, conds_t& status)
{
...
void
ma_condition::extract_fields(msg_t const& msg)
{
  sev_ = 0;
...
bool
ma_condition::match_body()
{
  if (match_type == MATCH_ANY) return true;
...
bool
ma_condition::match_test()
{
  return test_expr.evaluate(this);
}

bool
ma_condition::get_defined() const
{
  auto s_sz = hitmap.source_size();
  auto t_sz = hitmap.target_size();

  if (s_sz == 0 || t_sz == 0) return false;

  if (s_sz > 1 || t_sz > 1)
    throw std::runtime_error("ma_condition::get_defined() is only applicable "
                             "to non-parameterized conditions");
