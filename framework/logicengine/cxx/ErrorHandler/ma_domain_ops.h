...
namespace novadaq {
  namespace errorhandler {
    ...
    inline ma_cond_domain& domain_intersect(ma_cond_domain& d1,
                                            ma_cond_domain const& d2);

    // copy version
    inline ma_cond_domain domain_intersect_copy(ma_cond_domain d1,
                                                ma_cond_domain const& d2);

    // 2. (s_out, t_out) = (s1, t1) n (s2, t2) n ... n (s_n, t_n), in-place and
    // intersection of multiple cond_domain objects
    inline ma_cond_domain& domain_intersect(ma_cond_domains& d);

    // copy version
    inline ma_cond_domain domain_intersect_copy(ma_cond_domains d);

    // 3. intersection of two domain objects, result stores in the first
    inline ma_domain& domain_intersect(ma_domain& d1, ma_domain const& d2);

    // copy version
    inline ma_domain domain_intersect_copy(ma_domain d1, ma_domain const& d2);
    ...
    inline ma_domain& domain_intersect(ma_domains& d);

    // copy version
    inline ma_domain domain_intersect_copy(ma_domains d);
    ...
    enum domain_compare_result_t { NO_COMMON, EQUAL, L_CONTAINS, R_CONTAINS };

    inline domain_compare_result_t domain_compare(ma_cond_domain const& d1,
                                                  ma_cond_domain const& d2);

    // 1. add a new cond_domain to a list of cond_domains
    inline ma_cond_domains& domain_union(ma_cond_domains& ds,
    ...
    //     ...
    // * in-place union
    inline ma_domains& domain_union(ma_domains& ds);

    // utility functions
    inline bool domain_is_null(ma_cond_domain const& d);

    inline bool domain_is_null(ma_domain const& d);

    inline void domain_union_construct(ma_domains& ds,
                                       ma_domain& d,
    ...
    inline ma_cond_domain ma_cond_domain_ctor_null();

    // construct a any ma_cond_domain (*,*)
    inline ma_cond_domain ma_cond_domain_ctor_any();

    // construct a ma_cond_domain from s and t
    inline ma_cond_domain ma_cond_domain_ctor(int s, int t);

    // construct a ma_cond_domain from a value and arg type (source or target)
    // ends up with a ma_cond_domain of (i,*) or (*,i)
    inline ma_cond_domain ma_cond_domain_ctor(int i, arg_t t);

    // construct a null ma_domain
    inline ma_domain ma_domain_ctor_null();

    // construct a null ma_domain
    inline ma_domain ma_domain_ctor_null(size_t size);

    // construct a any ma_domain with size s
    inline ma_domain ma_domain_ctor_any(size_t size);

    // construct a ma_domain object with size s and init'd to d
    inline ma_domain ma_domain_ctor(size_t size, ma_cond_domain d);
                                        ...
                                        ma_cond_domain const& d2)
{
  ...
  if (domain_is_null(d1)) d1 = ma_cond_domain_ctor_null();
...
novadaq::errorhandler::domain_compare(ma_cond_domain const& d1,
                                      ma_cond_domain const& d2)
{
  if (d1.first == d2.first && d1.second == d2.second) return EQUAL;
  ...
  if (flag1 != flag2) return NO_COMMON;
                                    ...
                                    ma_cond_domain const& new_d)
{
  ...
  while (it != ds.end()) {
    ...
    if (r == EQUAL || r == L_CONTAINS) return ds;
...
void
novadaq::errorhandler::domain_union_construct(ma_domains& ds,
                                              ma_domain& d,
                                              std::vector<ma_cond_domains>& mid,
                                              int depth,
                                              int max)
