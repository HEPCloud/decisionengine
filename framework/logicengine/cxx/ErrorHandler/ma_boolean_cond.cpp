...
void
ma_boolean_cond::insert_ext_func(cond_idx_t ci,
                                 arg_t arg,
                          ...
                          ma_domain const& domain) const
{
  ...
  if (cond_type >= FUNCTION) {
    ...
    if (ext_func->grouped_alarm()) {
      ...
      alarm[cond_idx.second] = ma_cond_domain(
        cond_idx.first->find_source(cond_idx.first->get_msg_source()),
        cond_idx.first->find_target(cond_idx.first->get_msg_target()));
    }

    // evaluate
    boost::any v =
      ext_func->evaluate(*(cond_idx.first), domain[cond_idx.second]);
    double d;
    bool b;
    string_t s;

    switch (cond_type) {
    case FUNCTION: b = boost::any_cast<bool>(v); return b;
