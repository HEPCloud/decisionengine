...
namespace novadaq {
  namespace errorhandler {
    ...
    public:
      ...
      virtual boost::any evaluate(ma_condition const& cond,
    ...
    public:
      static void reg(std::string const& func_name, gen_func_t f);

      static ma_function* create_instance(std::string const& func_name);
...
#define REG_MA_FUNCTION(func_name, class_name)                                 \
  ma_function* class_name##_maker_func() { return new class_name(); }          \
  ma_function_maker class_name##_maker_func_global_var(                        \
