...
ma_domain_cond::ma_domain_cond()
  : cond_type(COND), conds(), str_cond(), cond_size(0), expr()
{}

void
ma_domain_cond::insert_expr(ma_domain_expr const& d_expr)
{
...
void
ma_domain_cond::evaluate(ma_domains& domains) const
{
  ...
  if (cond_type == EXPR) {
    if (expr.get() == NULL) throw std::runtime_error("expr not exist");
  ...
  // 2. cond_1.$ = cond_2.$ = ... = 'string literal'
    ...
    while (it != conds.end()) {
      ...
      if (idx == D_NIL) {
        ...
        domain[c_idx] = ma_cond_domain_ctor(idx, c_arg);
...
ma_domains&
ma_domain_cond::and_merge(ma_domains& domains, ma_domains& worksheet) const
{
  if (domains.empty()) {
