...
void
ma_cond_test_primary::insert_expr(ma_cond_test_expr const& e)
{
...
void
ma_cond_test_primary::insert_func(string_t const& name, anys_t const& args)
{
...
void
ma_cond_test_primary::insert_compare_op(compare_op_t cop, any_t const& v)
{
  op = cop;
...
bool
ma_cond_test_primary::evaluate(ma_condition const* cond) const
{
  ...
  if (cond_type == EXPR) {
    ...
    switch (cond_type) {
    case FUNCTION: return boost::any_cast<bool>(v);
    ...
    case FUNCTION_DOUBLE:
      if (v.type() == typeid(int))
        d = boost::any_cast<int>(v);
      else if (v.type() == typeid(unsigned int))
        d = boost::any_cast<unsigned int>(v);
      else if (v.type() == typeid(long))
        d = boost::any_cast<long>(v);
      else if (v.type() == typeid(float))
        d = boost::any_cast<float>(v);
      else
        d = boost::any_cast<double>(v);

      return compare(op, d, rhv_d);

    default: throw std::runtime_error("Unkonwn test primary type");
    }
  }
}
