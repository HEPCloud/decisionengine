...
namespace novadaq {
  namespace errorhandler {
    ...
    public:
      ...
      void
      insert_cond_arg(cond_idx_t ci, arg_t arg, size_t size)
      {
        conds.push_back(std::make_pair(ci, arg));
        cond_size = size;
      }

      // insert string condition ( cond.$s = 'str_cond' )
      void
      insert_str_cond(string_t const& str)
      {
        str_cond = str;
      }
      ...
      ma_domains& and_merge(ma_domains& domains, ma_domains& worksheet) const;
