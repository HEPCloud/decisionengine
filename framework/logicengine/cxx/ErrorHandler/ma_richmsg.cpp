...
namespace {
  ...
  bool
  parse_msg_ref(string_t const& s,
                ma_rule const* rule,
                std::vector<cond_arg_t>& symbols)
  {
    ...
    if (pos == s.size() || string_t("stmg").find(s[pos]) == string_t::npos)
      return false;
    ...
    case 'g': // $gn, group-n
      if (pos == s.size() - 2 && s[pos + 1] >= '1' && s[pos + 1] <= '9') {
        symbols.push_back(
          cond_arg_t(cond_idx, (arg_t)(GROUP1 + s[pos + 1] - '1')));
        return true;
      }
      return false;
            ...
            std::vector<cond_arg_t>& symbols)
  {
    ...
    while (pos != string_t::npos) {
      ...
      if (close == string_t::npos) return false; // no close '}'
    ...
    if (old < s.size()) stripped_msg.append(s.substr(old));
...
ma_richmsg::ma_richmsg()
  : rule(NULL), plain_msg(), stripped_msg(), insert_pos(), symbols()
{}
...
void
ma_richmsg::init(ma_rule const* parent, string_t const& s)
{
  rule = parent;
  plain_msg = s;
...
const string_t&
ma_richmsg::plain_message() const
{
  return plain_msg;
}

string_t
ma_richmsg::message() const
{
  ...
  for (int i = symbols.size() - 1; i >= 0; --i) {
    ...
    result.insert(insert_pos[i], cond_ptr->get_arg(alarm[cond_idx], cond_arg));
  }
