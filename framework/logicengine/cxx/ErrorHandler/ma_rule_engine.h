...
namespace novadaq {
  namespace errorhandler {
    ...
    public:
      ...
      void
      enable_rule(string_t const& name, bool flag)
      {
      ...
      void
      reset_rule(string_t const& name)
      {
      ...
      // reset conditions
      void
      reset_cond(string_t const& name)
      {
      ...
      // participants
      void
      add_participant_group(string_t const& group)
      {
        ma_participants::instance().add_group(group);
      }

      void
      add_participant_group(string_t const& group, size_t size)
      {
        ma_participants::instance().add_group(group, size);
      }

      void
      add_participant(string_t const& group, string_t const& app)
      {
        ma_participants::instance().add_participant(group, app);
      }

      void
      add_participant(string_t const& app)
      {
    ...
    private:
      ...
      void init_minimal_engine(Json::Value const& facts,
                               Json::Value const& rules);
