                            ...
                            size_t n)
{
  ...
  if (primitive_cond[n]) conditions[n]->get_cond_range(domain[n], src, target);
  ...
  for (int s = src.first; s <= src.second; ++s) {
    for (int t = target.first; t <= target.second; ++t) {
      ...
      if (n != domain.size() - 1) {
        if (recursive_evaluate(value, alarm, domain, n + 1)) return true;
      }
      else {
        // evaluate and, if found new alarm, no need to continue
        if (boolean_evaluate(value, alarm, domain)) return true;
      }
    }
  }
...
ma_rule::evaluate()
{
  ...
  for (ma_domains::const_iterator ait = domains.begin(); ait != domains.end();
       ++ait) {
    ...
    if (recursive_evaluate(value, alarm, domain, 0)) return true;
  }
...
ma_rule::act()
{
  ...
  int mask = 0;
  for (ma_actions::const_iterator it = actions.begin(), e = actions.end();
       it != e;
       ++it) {
    if ((*it)->exec()) ++mask;
  }
...
ma_rule::update_notify_list(string_t const& name, arg_t arg)
{
  ...
  if (it == conditions_idx.end())
    throw std::runtime_error("update_notify_list: name '" + name +
                             "' not found");
