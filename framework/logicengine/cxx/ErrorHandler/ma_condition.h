...
namespace novadaq {
  namespace errorhandler {
    ...
    public:
      ...
      bool force(bool val, conds_t& status, conds_t& source, conds_t& target);
      ...
      int
      find_source(string_t const& src)
      {
        return hitmap.find_source(src);
      }
      int
      find_target(string_t const& tgt)
      {
        return hitmap.find_target(tgt);
      }
      int
      find_arg(string_t const& arg, arg_t type)
      {
        return (type == SOURCE) ? hitmap.find_source(arg) :
                                  hitmap.find_target(arg);
      }
      ...
      const string_t&
      get_source(ma_cond_domain v) const
      {
        return hitmap.get_source(v);
      }
      const string_t&
      get_target(ma_cond_domain v) const
      {
        return hitmap.get_target(v);
      }
      string_t
      get_arg(ma_cond_domain v, arg_t type) const
      {
        ...
        if (type >= GROUP1)
          return hitmap.get_message_group(v, (size_t)(type - GROUP1 + 1));
      ...
      {
        push_notify(notify_on_source, rule);
      }
      void
      push_notify_target(ma_rule* rule)
      {
        push_notify(notify_on_target, rule);
      }
      void
      ...
      sort_notify_lists()
      {
        notify_on_source.sort();
        notify_on_target.sort();
