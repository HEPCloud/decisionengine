...
namespace novadaq {
  namespace errorhandler {
    ...
    public:
      ...
      // evaluation function
      virtual boost::any evaluate(ma_condition const& cond) = 0;
    ...
    public:
      static void reg(std::string const& func_name, gen_test_t f);

      static ma_test_function* create_instance(std::string const& func_name);
...
#define REG_MA_TEST_FUNCTION(func_name, class_name)                            \
  ma_test_function* class_name##_maker_func() { return new class_name(); }     \
  ma_test_function_maker class_name##_maker_func_global_var(                   \
    #func_name, class_name##_maker_func);
