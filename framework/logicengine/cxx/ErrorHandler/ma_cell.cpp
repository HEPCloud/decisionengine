             ...
             size_t t_idx)
{
  ...
  if (msgs.size() == (size_t)cond.trigger_count()) {
    ...
    cond.timing_events().event_queue().push(
...
bool
ma_cell::event(time_t t, ma_condition& cond)
{
  // not reached the event time, no flip
  if (t != t_event) return false;

  bool new_status = cond.at_most() ? true : (cond.persistent() ? true : false);

  // not flipped
  if (new_status == on) return false;
