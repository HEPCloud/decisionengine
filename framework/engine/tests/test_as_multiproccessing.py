...
class TestClientServerPython(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        de_client.main(args_to_parse=['--port=' + self.port, '--stop'])
        time.sleep(5)

        if self.server_proc.is_alive():
            self.server_proc.terminate()

        raise ValueError(multiprocessing.active_children())
    ...
    def test_client_can_get_de_server_reaper_stop(self):
        output = de_client.main(['--port', self.port, '--reaper-stop'])
        self.assertEqual('OK', output, msg="DE didn't say OK")
    ...
    def test_client_can_get_de_server_reaper_start_delay(self):
        ...
        de_client.main(['--port', self.port, '--reaper-stop'])

        output = de_client.main(['--port', self.port, '--reaper-start', '--reaper-start-delay-secs=90'])
        self.assertEqual('OK', output, msg="DE didn't say OK")
